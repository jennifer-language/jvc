...
import "../cli/catalog.j" as catalog;
import "../cli/resolver.j" as resolver;
...
export func addDeck(db as flatdb.DB, cat as catalog.Catalog, name as string) {
    ...
    for (def version in store.listVersions($db, $name)) {
        def rec as json.Value init store.getVersionJson($db, $name, $version);
        def kind as string init "tar.gz";
        ...
        $out = catalog.add($out, catalog.Candidate{
            ...
            requires: store.versionRequires($db, $name, $version),
            engines: store.versionEngines($db, $name, $version),
            capabilities: store.versionCapabilities($db, $name, $version)
        });
    }
    return $out;
}
